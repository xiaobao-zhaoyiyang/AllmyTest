...
public class SmsDbOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String sql = "create table " + SendMsg.TAB_NAME + " ( " +
                "_id integer primary key autoincrement , " +
                SendMsg.COLUMN_DATE + " integer , " +
                SendMsg.COLUMN_FESTIVAL_NAME + " text , " +
                SendMsg.COLUMN_MSG + " text , " +
                SendMsg.COLUMN_NAMES + " text , " +
                SendMsg.COLUMN_NUMBER + " text " + " ) ";
        db.execSQL(sql);
    }
